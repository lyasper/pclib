...
def syncexec_force(shellcmds, t=300, logcmd=False, logrst=False, outputfile=sys.stdout):
    if not shellcmds:
        return None
    myprt = prtwrapper(file=outputfile)
    if logcmd: myprt(shellcmds)
...
def syncexec_timeout(shellcmds, t=600, logcmd=False, logrst=False, outputfile=sys.stdout, retry=1):
    if not shellcmds:
        return None
    myprt = prtwrapper(file=outputfile)
    if logcmd: myprt(shellcmds)
    ...
    while (count > 0):
        ...
        if logrst:
            if ret:
                ...
                myprt(ret.error)
        if ret:
            # exit retry loop once success
            break
        count -= 1
        if count:
            myprt("{0} times to retry ...".format(count))
            time.sleep(0.5)
...
def main():
    ...
    #print "generator:"
    for i in syncexec_generater(cmd,t=10, logcmd=True, retry = 10):
        print i
    
    #print "normal:"
    r = syncexec_timeout(cmd,logcmd=True, logrst=True, retry = 3)
    #print r
